  ...
  for (quartt in quart){
    ...
    for (catt in categ){
      ...
      b + geom_point(stat = "identity") +  xlab("") + ylab("")   + ylim(c(0,max(this$CV))) + 
        ...
        geom_hline(aes(yintercept = quantile(this$CV, probs =c(0.05),na.rm=TRUE )),col=as.character("blue"),linetype="dashed") + 
        geom_hline(aes(yintercept = quantile(this$CV, probs =c(0.25) ,na.rm=TRUE)),col=as.character("blue"),linetype="dashed") + 
        geom_hline(aes(yintercept = quantile(this$CV, probs =c(0.5)  ,na.rm=TRUE)),col=as.character("blue"),linetype="dashed") + 
        geom_hline(aes(yintercept = quantile(this$CV, probs =c(0.75) ,na.rm=TRUE)),col=as.character("blue"),linetype="dashed") + 
        geom_hline(aes(yintercept = quantile(this$CV, probs =c(0.95) ,na.rm=TRUE)),col=as.character("blue"),linetype="dashed") +
        ...
        for (quartt in quart){
            for (catt in categ){
              ...
              b + geom_point(stat = "identity") +  xlab("") + ylab("") + facet_grid(.~samp_size)   + ylim(c(0,max(dataset2$pi))) + 
                theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
                ggtitle(paste(species_name, "- cat=",catt, "met=", mett, "quarter=", quartt))  
              ggsave(paste(res_dir,"/Cat ",catt,"_met",mett,"_Q",quartt,"_maturity_at_length_by_trip_", SEX,".jpg", sep=""), 
                     last_plot(),  height=8, width=16) 
              }
            }
